# -*- coding: utf-8 -*-
"""
IDCFlare / Discourse topic fetcher.
...
_IDCFLARE_DOMAINS = ("idcflare.com",)
...
def is_idcflare_url(url: str) -> bool:
    """Check whether *url* belongs to idcflare.com."""
    netloc = urlparse(url).netloc.lower()
    if netloc.startswith("www."):
        netloc = netloc[4:]
    return any(netloc == d or netloc.endswith("." + d) for d in _IDCFLARE_DOMAINS)


def parse_idcflare_url(url: str) -> Tuple[str, str, Optional[str]]:
    """Parse IDCFlare topic URL into (slug, topic_id, post_number)."""
    ...
    raise ValueError(f"不支持的 IDCFlare 帖子链接格式: {url}")


def _canonical_topic_url(url: str) -> str:
    slug, topic_id, _ = parse_idcflare_url(url)
    return f"https://idcflare.com/t/{slug}/{topic_id}"


def _topic_json_candidates(url: str) -> List[str]:
    slug, topic_id, _ = parse_idcflare_url(url)
    candidates = [
        f"https://idcflare.com/t/{slug}/{topic_id}.json",
        f"https://idcflare.com/t/{topic_id}.json",
    ]
...
def _session_path() -> Path:
    ...
    return get_session_dir() / "idcflare.json"


def _cookie_header_from_session() -> str:
    """Load idcflare.com cookies from Playwright storage_state."""
    ...
    try:
        ...
        logger.debug(f"[IDCFlare] session parse failed: {e}")
        return ""

    pairs = []
    for cookie in state.get("cookies", []):
        domain = cookie.get("domain", "")
        if domain.endswith(".idcflare.com") or domain == "idcflare.com":
            pairs.append(f"{cookie.get('name', '')}={cookie.get('value', '')}")
    return "; ".join(p for p in pairs if p and "=" in p)


def _has_idcflare_session_cookie() -> bool:
    return bool(_cookie_header_from_session().strip())


def _with_idcflare_login_guidance(message: str) -> str:
    """Append actionable login/CDP guidance for access-related failures."""
    if not message:
        return message
    if "请先运行 feedgrab login idcflare" in message:
        return message + "（也可设置 CHROME_CDP_LOGIN=true 后执行 feedgrab login idcflare 直接复用已登录 Chrome）"
    if "无权访问" in message or "需要登录" in message:
        return (
            message
            + " 请先运行 feedgrab login idcflare 保存会话，"
            + "或设置 CHROME_CDP_LOGIN=true 后再执行 feedgrab login idcflare。"
        )
    return message
...
def _clean_idcflare_title(title: str) -> str:
...
def _html_to_markdown(html: str) -> str:
    ...
    for tag in soup.find_all(["a", "img"]):
        ...
        if val.startswith("//"):
            ...
            tag[attr] = urljoin("https://idcflare.com", val)
...
def _extract_first_image(payload: Dict[str, Any]) -> str:
    ...
    if lightbox_match:
        ...
        if src.startswith("/"):
            return urljoin("https://idcflare.com", src)
        if src.startswith("//"):
            return "https:" + src
        return src
...
def _parse_topic_payload(payload: Dict[str, Any], url: str) -> Dict[str, Any]:
    ...
    from feedgrab.config import idcflare_reply_mode

    posts = payload.get("post_stream", {}).get("posts", [])
    if not posts:
        raise RuntimeError("IDCFlare topic JSON 中没有 posts 数据")

    canonical_url = _canonical_topic_url(url)
    op = posts[0]
    reply_mode = idcflare_reply_mode()
    ...
    content = "\n".join(lines).strip()
    return {
        "title": _clean_idcflare_title(payload.get("title", "")) or "Untitled",
        "content": content,
        "url": canonical_url,
        "author": author,
...
def _http_fetch_topic_json(url: str) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    ...
    for api_url in _topic_json_candidates(url):
        try:
            ...
            logger.debug(f"[IDCFlare] HTTP JSON failed: {e}")
            continue

        text = resp.text or ""
        is_cf_challenge = resp.headers.get("Cf-Mitigated", "").lower() == "challenge"
        if resp.status_code == 404:
            if saw_challenge:
                logger.info("[IDCFlare] HTTP JSON got 404 after challenge; trying next tier")
                continue
            return None, "IDCFlare 帖子不存在，或当前游客状态无权访问。"
        if resp.status_code in (401, 403) and (_looks_like_challenge(text) or is_cf_challenge):
            logger.info("[IDCFlare] HTTP JSON hit Cloudflare challenge")
            saw_challenge = True
            continue
        if resp.status_code in (401, 403):
            return None, "IDCFlare 帖子需要登录后才能抓取。请先运行 feedgrab login idcflare。"
        if resp.status_code != 200:
            logger.debug(f"[IDCFlare] HTTP JSON status={resp.status_code}")
            continue
        if _looks_like_challenge(text):
            logger.info("[IDCFlare] HTTP JSON returned challenge page")
            saw_challenge = True
            continue
        if _looks_like_not_found(text):
            if saw_challenge:
                logger.info("[IDCFlare] HTTP JSON looked like not-found after challenge; trying next tier")
                continue
            return None, "IDCFlare 帖子不存在，或当前游客状态无权访问。"
        ...
        try:
            ...
            logger.debug("[IDCFlare] HTTP JSON response was not valid JSON")
            continue
...
async def _connect_idcflare_cdp() -> Optional[tuple]:
    """Connect to running Chrome via CDP and reuse a context with idcflare.com cookies."""
    ...
    try:
        ...
        logger.debug(f"[IDCFlare] CDP connected: {ws_url}")
        for ctx in browser.contexts:
            cookies = await ctx.cookies()
            if any(
                c.get("domain", "").endswith(".idcflare.com") or c.get("domain", "") == "idcflare.com"
                for c in cookies
            ):
                await _save_idcflare_cookies(cookies)
                page = await ctx.new_page()
                logger.info("[IDCFlare] CDP: reusing existing Chrome idcflare.com session")
                return pw, browser, ctx, page
        await browser.close()
        await pw.stop()
    except Exception as e:
        logger.debug(f"[IDCFlare] CDP connect failed: {e}")
    return None


async def _save_idcflare_cookies(cookies: List[Dict[str, Any]]) -> None:
    session_path = _session_path()
    session_path.parent.mkdir(parents=True, exist_ok=True)
    idcflare_cookies = [
        cookie for cookie in cookies
        if cookie.get("domain", "").endswith(".idcflare.com") or cookie.get("domain", "") == "idcflare.com"
    ]
    if not idcflare_cookies:
        return
    session_path.write_text(
        json.dumps({"cookies": idcflare_cookies, "origins": []}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    try:
...
async def _seed_idcflare_session_via_browser() -> bool:
    """Best-effort seed of idcflare.json via a short browser visit."""
    pw = browser = context = page = None
    try:
        pw, browser, context, page = await _launch_idcflare_browser()
        ...
        if _has_idcflare_session_cookie():
            return True
        ...
        logger.debug(f"[IDCFlare] browser session seed failed: {e}")
        return False
    finally:
        try:
            if page:
...
async def _warmup_idcflare_session_from_cdp() -> bool:
    """If local session is missing, try CDP sync first, then browser seed."""
    session_path = _session_path()
    if session_path.exists() and _has_idcflare_session_cookie():
        return True
    if session_path.exists() and not _has_idcflare_session_cookie():
        try:
            session_path.unlink()
        except OSError:
            pass

    from feedgrab.config import idcflare_cdp_enabled

    if idcflare_cdp_enabled():
        logger.info("[IDCFlare] session warmup: idcflare.json missing, trying CDP cookie sync")
        pw = browser = page = None
        try:
            cdp = await _connect_idcflare_cdp()
            if not cdp:
                logger.debug("[IDCFlare] session warmup: no CDP idcflare.com context found")
            else:
                pw, browser, _ctx, page = cdp
                if session_path.exists():
                    return True
        except Exception as e:
            logger.debug(f"[IDCFlare] session warmup failed: {e}")
        finally:
            try:
                if page:
                    await page.close()
    ...
    logger.info("[IDCFlare] session warmup: trying browser guest session seed")
    seeded = await _seed_idcflare_session_via_browser()
    if seeded:
        logger.info("[IDCFlare] session warmup: idcflare.json seeded")
    else:
        logger.warning("[IDCFlare] session warmup: unable to create idcflare.json")
    return session_path.exists() and _has_idcflare_session_cookie()


async def _launch_idcflare_browser():
...
async def _wait_idcflare_page_ready(page) -> Dict[str, str]:
...
async def _fetch_topic_json_in_page(page, url: str) -> Dict[str, Any]:
    ...
    from feedgrab.config import idcflare_page_load_timeout

    topic_url = _canonical_topic_url(url)
    await page.goto(
        topic_url,
        wait_until="domcontentloaded",
        timeout=idcflare_page_load_timeout(),
        referer=generate_referer(topic_url),
    )
    page_status = await _wait_idcflare_page_ready(page)
    ...
    if _looks_like_not_found(title) or _looks_like_not_found(body):
        raise RuntimeError("IDCFlare 帖子不存在，或当前游客状态无权访问。")
    if "登录" in body or "login" in body.lower():
        raise RuntimeError("IDCFlare 帖子需要登录后才能抓取。请先运行 feedgrab login idcflare。")
    raise RuntimeError("IDCFlare 浏览器态 JSON 抓取失败，可能仍被 Cloudflare 拦截。")


async def fetch_idcflare(url: str) -> Dict[str, Any]:
    """Fetch a IDCFlare topic with Discourse-aware multi-tier fallbacks."""
    if not is_idcflare_url(url):
        raise ValueError(f"不是 IDCFlare 链接: {url}")

    # Optional warmup — first successful CDP sync writes idcflare.json for later reuse.
    has_local_session = await _warmup_idcflare_session_from_cdp()

    # Tier 0 — HTTP JSON with guest/session cookies
    logger.info("[IDCFlare] Tier 0 — Discourse topic JSON")
    ...
    if terminal_error:
        ...
        logger.warning(f"[IDCFlare] Tier 0 terminal error: {terminal_error}")

    # Tier 1 — CDP: reuse running Chrome session / cf_clearance
    from feedgrab.config import idcflare_cdp_enabled

    if idcflare_cdp_enabled():
        logger.info("[IDCFlare] Tier 1 — CDP browser reuse")
        cdp = await _connect_idcflare_cdp()
        if cdp:
            ...
            try:
                ...
                logger.warning(f"[IDCFlare] CDP fetch failed: {e}")
                if terminal_error:
                    raise RuntimeError(_with_idcflare_login_guidance(terminal_error)) from e
            finally:
                try:
                    if page:
                        await page.close()
                ...
                try:
                    ...
                    pass
        elif not has_local_session:
            logger.warning(
                "[IDCFlare] Tier 1 未发现可复用登录态。可先执行 feedgrab login idcflare "
                "（或设置 CHROME_CDP_LOGIN=true 后执行同一命令）。"
            )

    # Tier 2 — Stealth Playwright launch
    logger.info("[IDCFlare] Tier 2 — Stealth browser launch")
    pw = browser = context = page = None
    try:
        pw, browser, context, page = await _launch_idcflare_browser()
        data = await _fetch_topic_json_in_page(page, url)
        ...
        logger.warning(f"[IDCFlare] Tier 2 failed: {e}")
        if terminal_error:
            raise RuntimeError(_with_idcflare_login_guidance(terminal_error)) from e
    finally:
        try:
    ...
    if not has_local_session and terminal_error:
        raise RuntimeError(_with_idcflare_login_guidance(terminal_error))

    # Tier 3 — Jina fallback only for non-terminal challenge/network failures
    logger.info("[IDCFlare] Tier 3 — Jina fallback")
    ...
    return {
        ...
        "category_id": 0,
        "tags": [],
        "topic_id": parse_idcflare_url(url)[1],
        "topic_slug": parse_idcflare_url(url)[0],
        "posts_count": 1,
        "reply_count": 0,
        "like_count": 0,
        "views": 0,
